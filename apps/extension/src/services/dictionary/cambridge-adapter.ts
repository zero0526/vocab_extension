...
export class CambridgeAdapter implements DictionaryClient {
  ...
  private async fetchFromBackend(word: string): Promise<DictionaryLookupResult | null> {
    ...
    try {
      ...
      if (res.ok) {
        return (await res.json()) as DictionaryLookupResult;
      }
    } catch {
      // ignore and fallback
    } finally {
  ...
  private async fetchDirectCambridge(word: string): Promise<DictionaryLookupResult> {
    ...
    defEls.forEach((block, idx) => {
      if (idx > 4) return; // Limit to top 5 definitions for capture
      ...
      exEls.forEach((exEl, exIdx) => {
        if (exIdx > 1) return; // top 2 per def
